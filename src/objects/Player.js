...
export default class Player extends Phaser.Physics.Arcade.Sprite {
  constructor(scene, x, y) {
    ...
    this.hairSprite = scene.add
      .sprite(x, y, "player_hair_idle", 0)
      .setScale(1)
      .setOrigin(0.5, 38 / 64)
      .setDepth(6);
    this.toolSprite = scene.add
      .sprite(x, y, "player_tools_idle", 0)
      .setScale(1)
      .setOrigin(0.5, 38 / 64)
      .setDepth(7)
      .setVisible(false);
  ...
  obterMultiplicadorEnergia() {
    ...
    } else if (percentagem >= 0.1) {
      return 0.7; // Cansado: 70% da capacidade
    } else {
      return 0.5; // Exausto: 50% da capacidade
    }
  }
  ...
  update(cursors, wasd, time, delta) {
    ...
    const speed = running
      ? this.speedRun
      : this.speedWalk * this.obterMultiplicadorEnergia();

    // Consome energia continuamente enquanto corre
    if (running && this.scene.stats) {
      this.scene.stats.energy = Math.max(
        0,
    ...
    if (dx !== 0 || dy !== 0) {
      ...
      if (dx < 0) {
        this.atualizarDirecao(true);
      } // Vira para a esquerda
      else if (dx > 0) {
        this.atualizarDirecao(false);
      } // Vira para a direita
    ...
    if (this.scene.stats) {
      if (this.scene.stats.energyPercentagem < 0.3) {
    ...
    goblins.forEach((g) => {
      if (!g.dead) {
        ...
        if (d < this.attackRange) {
          ...
          g.takeDamage(
            this.attackDamage * multiplicador,
            this.facingX < 0 ? "left" : "right",
          );
