...
export default class HUDScene extends Phaser.Scene {
  ...
  criarPainelQuestTimer(W, H) {
    ...
    graficoQuest.fillRoundedRect(QPX, QPY, QPW, QPHH, {
      tl: 6,
      tr: 6,
      bl: 0,
      br: 0,
    ...
    this.add
      .text(QPX + QPW / 2, QPY + QPHH / 2, "JANGADA  [F]", {
        fontSize: "9px",
        fill: "#f5c070",
        fontStyle: "bold",
      })
    ...
    this.timerTxt = this.add
      .text(QPX + QPW / 2 + 8, QSEP + 6, "0:00", {
        fontSize: "13px",
        fill: "#ffffff",
        fontStyle: "bold",
      })
    ...
    this.dayNightIcon = this.add
      .text(QPX + QPW / 2 - 12, QSEP + 6, "☀️", {
        fontSize: "13px",
      })
      .setOrigin(0.5, 0)
      .setDepth(3);
    ...
    if (this.quest) {
      this.quest.getProgress().forEach((p, i) => {
        ...
        // Nome do recurso
        this.add
          .text(QPX + 28, rowCY, p.label, { fontSize: "9px", fill: "#ccbb88" })
          .setOrigin(0, 0.5)
          .setDepth(3);
        ...
        for (let s = 0; s < total; s++) {
          ...
          const r = this.add
            .rectangle(sx, rowCY, SLOT_TAMANHO, SLOT_TAMANHO, cfg.emptyColor)
  ...
  criarQuestLog(W, H) {
    ...
    // Fundo do Quest Log
    const graficoFundoLog = this.add
      .graphics()
      .setDepth(60)
      .setScrollFactor(0)
      .setVisible(false);
    ...
    graficoFundoLog.fillRoundedRect(cx, cy, painelW, HEADER_H, {
      tl: 6,
      tr: 6,
      bl: 0,
      br: 0,
    ...
    const titulo = this.add
      .text(cx + painelW / 2, cy + HEADER_H / 2, "REGISTO DA JANGADA", {
        fontSize: "11px",
        fill: "#f5c070",
        fontStyle: "bold",
      })
    ...
    const dicaFechar = this.add
      .text(W / 2, H / 2 + painelH / 2 - 14, "[Q] fechar", {
        fontSize: "9px",
        fill: "#c8a870",
        stroke: "#000000",
        strokeThickness: 2,
    ...
    if (this.quest) {
      this.quest.getProgress().forEach((p, i) => {
        ...
        const sub = this.add
          .text(iconX, y + 18, `${z.fonte} · ${z.zona}`, {
            fontSize: "10px",
            fill: "#8899aa",
          })
          .setOrigin(0, 0.5)
  ...
  criarDiarioNaufrago(W, H) {
    ...
    this.diarioTitulo = this.add
      .text(cx, cy - livroH / 2 + HEADER_H / 2 + 2, "", {
    ...
    this.diarioDivGrafico = this.add
      .graphics()
      .setDepth(D + 2)
      .setVisible(false);
    ...
    this.diarioTextoEsquerda = this.add
      .text(esquerdaX, textoY, "", {
        fontFamily: "Georgia, serif",
        fontSize: "12.5px",
        color: "#2e1c06",
        lineSpacing: 5,
    ...
    this.diarioTextoDireita = this.add
      .text(direitaX, textoY, "", {
        fontFamily: "Georgia, serif",
        fontSize: "12.5px",
        color: "#2e1c06",
        lineSpacing: 5,
    ...
    this.diarioNumeroPagina = this.add
      .text(cx, cy + livroH / 2 - 18, "", {
        fontFamily: "Georgia, serif",
        fontSize: "11px",
        color: "#8b6e3a",
        align: "center",
    ...
    const larguraBotao = 48,
      alturaBotao = 24;
    const prevX = cx - livroW / 2 + 38;
    const nextX = cx + livroW / 2 - 38;

    // Botão Página Anterior
    this.diarioBtnAnteriorGrafico = this.add
      .graphics()
      .setDepth(D + 2)
      .setVisible(false);
    this.diarioBtnAnteriorTexto = this.add
      .text(prevX, navY, "◀", {
        fontFamily: "Georgia, serif",
        fontSize: "14px",
        color: "#6b4010",
    ...
    this.diarioBtnAnteriorZona = this.add
      .zone(prevX, navY, larguraBotao, alturaBotao)
      .setInteractive({ useHandCursor: true })
      .setDepth(D + 4)
      .setVisible(false);
    this.diarioBtnAnteriorZona.on("pointerover", () =>
      this.hoverBotaoNavegacao("prev", true),
    );
    this.diarioBtnAnteriorZona.on("pointerout", () =>
      this.hoverBotaoNavegacao("prev", false),
    );
    this.diarioBtnAnteriorZona.on("pointerdown", () =>
      this.virarPaginaDiario(-1),
    );

    // Botão Página Seguinte
    this.diarioBtnSeguinteGrafico = this.add
      .graphics()
      .setDepth(D + 2)
      .setVisible(false);
    this.diarioBtnSeguinteTexto = this.add
      .text(nextX, navY, "▶", {
        fontFamily: "Georgia, serif",
        fontSize: "14px",
        color: "#6b4010",
    ...
    this.diarioBtnSeguinteZona = this.add
      .zone(nextX, navY, larguraBotao, alturaBotao)
      .setInteractive({ useHandCursor: true })
      .setDepth(D + 4)
      .setVisible(false);
    this.diarioBtnSeguinteZona.on("pointerover", () =>
      this.hoverBotaoNavegacao("next", true),
    );
    this.diarioBtnSeguinteZona.on("pointerout", () =>
      this.hoverBotaoNavegacao("next", false),
    );
    this.diarioBtnSeguinteZona.on("pointerdown", () =>
      this.virarPaginaDiario(1),
    );

    // Dica de rodapé de como fechar
    this.diarioDicaFechar = this.add
      .text(cx, cy + livroH / 2 + 16, "", {
        fontFamily: "Georgia, serif",
        fontSize: "10px",
        color: "#7a6040",
        fontStyle: "italic",
    ...
    this.bookGroup.addMultiple([
      this.diarioDim,
      this.diarioGrafico,
      this.diarioDivGrafico,
      this.diarioTitulo,
      this.diarioTextoEsquerda,
      this.diarioTextoDireita,
      this.diarioNumeroPagina,
      this.diarioBtnAnteriorGrafico,
      this.diarioBtnAnteriorTexto,
      this.diarioBtnAnteriorZona,
      this.diarioBtnSeguinteGrafico,
      this.diarioBtnSeguinteTexto,
      this.diarioBtnSeguinteZona,
  ...
  desenharGraficosDiario(cx, cy, livroW, livroH, headerH) {
    ...
    g.fillRoundedRect(cx - livroW / 2, cy - livroH / 2, livroW, headerH, {
      tl: r,
      tr: r,
      bl: 0,
      br: 0,
    ...
    g.fillRoundedRect(cx + 6, topoPagina, livroW / 2 - 16, alturaPagina, {
      tl: 0,
      bl: 0,
      tr: 0,
      br: r - 2,
    ...
    g.strokeRoundedRect(cx + 6, topoPagina, livroW / 2 - 16, alturaPagina, {
      tl: 0,
      bl: 0,
      tr: 0,
      br: r - 2,
  ...
  hoverBotaoNavegacao(lado, ativo) {
    if (lado === "prev") {
      this.desenharBotaoNavegacao(
        this.diarioBtnAnteriorGrafico,
        this.diarioBtnAnteriorTexto.x,
        this.diarioBtnAnteriorTexto.y,
        48,
        24,
        ativo,
      );
      this.diarioBtnAnteriorTexto.setStyle({
        color: ativo ? "#f0c060" : "#6b4010",
      });
    } else {
      this.desenharBotaoNavegacao(
        this.diarioBtnSeguinteGrafico,
        this.diarioBtnSeguinteTexto.x,
        this.diarioBtnSeguinteTexto.y,
        48,
        24,
        ativo,
      );
      this.diarioBtnSeguinteTexto.setStyle({
        color: ativo ? "#f0c060" : "#6b4010",
      });
    }
  }
  ...
  criarOverlayAlertaVida(W, H) {
    this.criticalOverlay = this.add
      .rectangle(W / 2, H / 2, W, H, 0xff0000, 0)
      .setDepth(50);
    this.criticalTxt = this.add
      .text(W / 2, 80, "⚠ VIDA CRÍTICA", {
        fontSize: "16px",
        fill: "#ff4444",
        fontStyle: "bold",
      })
  ...
  criarMinimap(W, H) {
    ...
    this.minimapBgGfx.fillRoundedRect(
      miniX - 4,
      miniY - 4,
      miniW + 8,
      miniH + 8,
    ...
    this.minimapBorderGfx.strokeRoundedRect(
      miniX - 1,
      miniY - 1,
      miniW + 2,
      miniH + 2,
    ...
    this.minimapMText = this.add
      .text(miniX + miniW / 2, miniY + miniH - 12, "MAP [M]", {
        fontSize: "8px",
        fill: "#ffffff",
        stroke: "#000000",
        strokeThickness: 2,
  ...
  update() {
    ...
    if (this.stats && !this.stats.dead) {
      const vidaCritica = this.stats.healthPercentagem < 0.2;
      if (vidaCritica) {
        if (!this.piscando) {
          this.piscando = true;
    ...
    if (this.minimapVisible && this.gameScene) {
      ...
      if (
        !this.terrenoMinimapDesenhado &&
        this.gameScene.colisao &&
        this.gameScene.mapa
      ) {
  ...
  iniciarPiscarAlertaVida() {
    this.criticalOverlay.setAlpha(0);
    this.criticalTxt.setAlpha(0);
    this.criticalTxt.setText(
      I18n.lang === "pt" ? "⚠ VIDA CRÍTICA" : "⚠ LOW HEALTH",
    );
    ...
    if (this.questLogAberto) {
      this.linhasQuestLog.forEach((linha) => {
        ...
        if (prog) {
          ...
          linha.prog.setStyle({
            fill: prog.have >= prog.need ? "#88ff88" : "#cccccc",
          });
          linha.head.setStyle({
            fill: prog.have >= prog.need ? "#88ff88" : "#ffffff",
          });
        }
      });
    }
  ...
  atualizarPaginaDiario() {
    const dadosIdioma = this.cache.json.get(
    ...
    const textoEsq = paginas[idxEsquerdo]
      ? paginas[idxEsquerdo].text.join("\n")
      : "";
    this.diarioTextoEsquerda.setText(textoEsq);

    // Repõe textos da página direita
    const textoDir = paginas[idxDireito]
      ? paginas[idxDireito].text.join("\n")
      : "";
    ...
    this.diarioNumeroPagina.setText(
      `${idxEsquerdo + 1} - ${Math.min(totalPags, idxDireito + 1)}  /  ${totalPags}`,
    );
    ...
    for (let ty = 0; ty < mapHeightInTiles; ty++) {
      for (let tx = 0; tx < mapWidthInTiles; tx++) {
        ...
        if (ehAgua) {
          ...
          this.minimapTerrainGfx.fillRect(
            miniX + tx * tileW,
            miniY + ty * tileH,
            tileW + 0.1,
            tileH + 0.1,
